...
class QuestionModelTest(TestCase):
    def test_was_published_recently_with_future_question(self):
        ...
        self.assertIs(future_question.was_pusblished_recently(), False)
        
    def test_was_published_recently_with_old_question(self):
        time = timezone.now() - timedelta(days=1,seconds=1)    
        old_question = Question(pub_date=time)
        self.assertIs(old_question.was_pusblished_recently(), False)
        
        
    def test_was_published_recently_with_recent_question(self):
        time =timezone.now() - timedelta(hours=23,minutes=59,seconds=59)
        recent_question = Question(pub_date=time)
        self.assertIs(recent_question.was_pusblished_recently(),True)   
...
class QuestionIndezTest(TestCase):
    ...
    def test_future_question(self):
        ...
        response =self.client.get(reverse('polls:index'))
        self.assertContains(response.context['No polls are available'], [])
        self.assertQuerysetEqual(response.context['lastest_question_list'],[])
